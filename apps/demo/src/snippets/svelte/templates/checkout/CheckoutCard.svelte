<script lang="ts">
	type Line = { name: string; detail: string; qty: number; price: string }
	type Props = {
		lines?: Line[]
		subtotal?: string
		shipping?: string
...
<div class="rounded-box bg-base-200/50 p-4 sm:p-6">
			...
			<div class="card-body gap-6">
				...
				<div>
					...
					<div class="mt-3 space-y-2">
						<label
							class="flex cursor-pointer items-center gap-3 rounded-box border border-base-300 bg-base-100 p-3 has-checked:border-primary has-checked:bg-primary/5"
						>
							<input
								type="radio"
			...
			<div class="card-body gap-4">
				<div class="flex items-center gap-2">
					<svg
						...
						stroke-width="2"
						aria-hidden="true"
						><path d="M16 10a4 4 0 0 1-8 0" /><path d="M3.103 6.034h17.794" /><path
				...
				<ul class="space-y-3">
					{#each lines as item}
						<li class="flex gap-3 text-sm">
							<div
								class="flex size-12 shrink-0 items-center justify-center rounded-box bg-primary/10"
							>
								<svg
									class="size-5 text-primary"
									viewBox="0 0 24 24"
							...
							<div class="min-w-0 flex-1">
								...
								<p class="text-ink-muted">{item.detail}</p>
								<p class="text-ink-muted">Qty {item.qty}</p>
							</div>

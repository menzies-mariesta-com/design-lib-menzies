<script lang="ts">
	...
	let activeSection = $state<DocSectionId>('install')
</script>
...
<div class="rounded-box border border-base-300 bg-base-100 shadow-sm">
	<div class="flex min-h-[22rem] flex-col lg:min-h-[28rem] lg:flex-row">
		<aside
			class="shrink-0 border-b border-base-300 bg-base-200/40 lg:sticky lg:top-0 lg:w-52 lg:self-start lg:border-b-0 lg:border-e"
		>
				...
				<ul class="menu menu-sm w-full gap-0.5 p-0">
					{#each sidebarLinks as link}
						<li>
							<button
								type="button"
								class="cursor-pointer rounded-btn px-3 {activeSection === link.id
									? 'bg-primary/40 font-medium'
									: ''}"
								aria-current={activeSection === link.id ? 'true' : undefined}
								onclick={() => (activeSection = link.id)}
							>
								{link.label}
							</button>
						</li>
		...
		<div class="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6 lg:max-h-[28rem]">
			<article class="mx-auto max-w-prose space-y-8">
				<header>
					...
					<h1 class="font-display text-xl font-semibold tracking-tight sm:text-2xl">
						Studio pigment tokens
					</h1>
					<p class="mt-2 text-sm leading-relaxed text-ink-muted">
						Map wash pigments to CSS variables and paper modes. Use this layout for long-form docs
						with a sticky sidebar and scroll-synced highlights.
					</p>
				</header>

				<section
					id="docs-install"
					class="scroll-mt-4 border-s-4 ps-4 {activeSection === 'install'
						? 'border-primary'
						: 'border-transparent'}"
				>
					...
					<pre
						class="mt-3 overflow-x-auto rounded-box bg-base-200 p-3 font-mono text-xs"
					><code>npm i @menzies-mariesta-com/menzies-design-wash-ui</code></pre>
					<div class="alert alert-info mt-3 text-sm shadow-sm">
						<span
							>React apps also need <code class="font-mono text-xs">react</code> and
							<code class="font-mono text-xs">react-dom</code> as peers.</span
						>
					</div>
				</section>
				...
				<section
					id="docs-theming"
					class="scroll-mt-4 border-s-4 ps-4 {activeSection === 'theming'
						? 'border-primary'
						: 'border-transparent'}"
				>
					...
					<p class="mt-2 text-sm leading-relaxed text-base-content/90">
						...
						<code class="font-mono text-xs">:root</code>.
					</p>
					<pre
						class="mt-3 overflow-x-auto rounded-box bg-base-200 p-3 font-mono text-xs"
					><code>:root &#123;
...
&#125;</code></pre>
				...
				<section
					id="docs-components"
					class="scroll-mt-4 border-s-4 ps-4 {activeSection === 'components'
						? 'border-primary'
						: 'border-transparent'}"
				>
					<h2 class="font-display text-lg font-semibold">Components</h2>
					<p class="mt-2 text-sm leading-relaxed text-base-content/90">
						Compose buttons, cards, and form controls from documented HTML classes or React exports.
					</p>
				</section>

				<section
					id="docs-charts"
					class="scroll-mt-4 border-s-4 ps-4 {activeSection === 'charts'
						? 'border-primary'
						: 'border-transparent'}"
				>
				...
				<section
					id="docs-api"
					class="scroll-mt-4 border-s-4 ps-4 {activeSection === 'api'
						? 'border-primary'
						: 'border-transparent'}"
				>
					<h2 class="font-display text-lg font-semibold">API</h2>
					<p class="mt-2 text-sm leading-relaxed text-base-content/90">
						Deep imports for theme, icons, and core utilities.
					</p>
					<pre
						class="mt-3 overflow-x-auto rounded-box bg-base-200 p-3 font-mono text-xs"
